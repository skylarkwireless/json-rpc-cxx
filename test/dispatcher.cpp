...
TEST_CASE("add and invoke positional") {
  Dispatcher d;
  CHECK(d.Add("some method", GetHandle("add_function", {"a", "b"}, &add_function)));
  CHECK(!d.Add("some method", GetHandle("add_function", {"a", "b"}, &add_function)));
  CHECK(d.InvokeMethod("some method", {11, 22}) == 33);

  procCache = "";
  CHECK(d.Add("some notification", GetHandle("some_procedure", {"param"}, &some_procedure)));
  CHECK(!d.Add("some notification", GetHandle("some_procedure", {"param"}, &some_procedure)));
...
TEST_CASE("invoking supported named parameter") {
  ...
  procCache = "";
  CHECK(d.Add("some notification", GetHandle("some_procedure", {"param"}, &some_procedure), {"param"}));
...
TEST_CASE("invoking missing named parameter") {
  Dispatcher d;
  CHECK(d.Add("some method", GetHandle("add_function", {"a", "b"}, &add_function), {"a", "b"}));
  REQUIRE_THROWS_WITH(d.InvokeMethod("some method", {{"a", 11}, {"xx", 22}}), "some method: missing named parameter \"b\"");

  procCache = "";
  CHECK(d.Add("some notification", GetHandle("some_procedure", {"param"}, &some_procedure), {"param"}));
  json p = {{"param2", "some string"}};
  REQUIRE_THROWS_WITH(d.InvokeNotification("some notification", p), "some notification: missing named parameter \"param\"");
...
TEST_CASE("error on invoking unsupported named parameter") {
  Dispatcher d;
  CHECK(d.Add("some method", GetHandle("add_function", {"a", "b"}, &add_function)));
  REQUIRE_THROWS_WITH(d.InvokeMethod("some method", {{"a", 11}, {"b", 22}}), "some method: procedure doesn't support named parameter");

  CHECK(d.Add("some notification", GetHandle("some_procedure", {"param"}, &some_procedure)));
  json p = {{"param", "some string"}};
  REQUIRE_THROWS_WITH(d.InvokeNotification("some notification", p), "some notification: procedure doesn't support named parameter");
}

TEST_CASE("passing invalid literal as param") {
    Dispatcher d;
    CHECK(d.Add("some method", GetHandle("add_function", {"a", "b"}, &add_function)));
    REQUIRE_THROWS_WITH(d.InvokeMethod("some method", true), "some method: invalid request (params field must be an array or object)");
}

TEST_CASE("dispatching unknown procedures") {
    Dispatcher d;
    REQUIRE_THROWS_WITH(d.InvokeMethod("some method", {1}), "method not found: some method");
    REQUIRE_THROWS_WITH(d.InvokeNotification("some notification", {1}), "notification not found: some notification");
}
...
TEST_CASE("invalid param types") {
    Dispatcher d;
    CHECK(d.Add("some method", GetHandle("add_function", {"a", "b"}, &add_function)));
    CHECK_THROWS_WITH(d.InvokeMethod("some method", {"string1", "string2"}), "some method: invalid parameter: must be unsigned integer, but is string for parameter 0");
}
*/
